...
struct  HephAccelerationStructureWrapper {
  VkAccelerationStructureKHR  accel = VK_NULL_HANDLE;
  HephBufferWrapper           buffer;
};
...
class		HephMemoryAllocator {
    ...
    VkDeviceAddress     getBufferDeviceAddress(HephBufferWrapper& buffer) {
      VkBufferDeviceAddressInfo addrInfo = {
        .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
        .buffer = buffer.buffer,
      };

      return (vkGetBufferDeviceAddress(m_device.device, &addrInfo));
    }

    VkDeviceAddress     getAccelerationStructureDeviceAddress(HephAccelerationStructureWrapper& accel) {
      VkAccelerationStructureDeviceAddressInfoKHR addrInfo = {
        .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_DEVICE_ADDRESS_INFO_KHR,
        .accelerationStructure = accel.accel,
      };

      return (vkGetAccelerationStructureDeviceAddressKHR(m_device.device, &addrInfo));
    }
